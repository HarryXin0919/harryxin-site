...
const RESEARCH_COHORTS = new Set(["pilot", "confirmation", "exploratory", "mechanism"]);
const RESEARCH_PROTOCOL_MODES = new Set([
  "post_outcome_exploratory",
  "post_outcome_mechanism_screening",
]);
const RESEARCH_SELECTION_STATES = new Set(["no_candidate_promoted"]);
...
const MECHANISM_STUDY_ID = "leduc-reward-mechanism-scale-pbrs-v1";
const MECHANISM_SOURCE_STUDY_ID = "leduc-reward-exploratory-scaled-v1";
const MECHANISM_PHASE_LABELS = new Map([
  [8, "REWARD-SCALE MECHANISM STUDY"],
  [9, "REPORT MECHANISM RESULTS"],
]);
const MECHANISM_ARMS = new Set([
  "terminal",
  "scaled",
  "scaled-pbrs-cfr-a025",
  "unscaled-pbrs-cfr-a175",
...
function sanitizeResearchMilestones(value) {
  if (!Array.isArray(value) || value.length > 9) {
    throw new StatusValidationError(
      "research.milestones must contain at most nine entries",
    );
  }
  return value.map((item, index) => {
...
function sanitizeResearchSelection(value) {
  if (value === null || value === undefined) return null;
  const selection = plainObject(value, "research.selection");
  const status = enumValue(
    selection.status,
    "research.selection.status",
    RESEARCH_SELECTION_STATES,
  );
  if (selection.selectedArm !== null) {
    throw new StatusValidationError(
      "research.selection.selectedArm must be null when no candidate was promoted",
    );
  }
  return {
    status,
    selectedArm: null,
    exploratoryArm: enumValue(
      selection.exploratoryArm,
      "research.selection.exploratoryArm",
      RESEARCH_ARMS,
    ),
  };
}
...
function sanitizeResearch(value) {
  ...
  const sanitized = {
    ...
    phase: integer(research.phase, "research.phase", { min: 1, max: 9 }),
  ...
  if (research.cohort !== null && research.cohort !== undefined) {
    sanitized.cohort = enumValue(
      research.cohort,
      "research.cohort",
      RESEARCH_COHORTS,
    );
  }
  if (research.protocolMode !== null && research.protocolMode !== undefined) {
    sanitized.protocolMode = enumValue(
      research.protocolMode,
      "research.protocolMode",
      RESEARCH_PROTOCOL_MODES,
    );
  }
  if (research.confirmatory !== null && research.confirmatory !== undefined) {
    sanitized.confirmatory = boolean(research.confirmatory, "research.confirmatory");
  }
  if (research.sourceStudyId !== null && research.sourceStudyId !== undefined) {
    sanitized.sourceStudyId = text(
      research.sourceStudyId,
      "research.sourceStudyId",
      { maxLength: 80 },
    );
  }
  if (research.selection !== null && research.selection !== undefined) {
    sanitized.selection = sanitizeResearchSelection(research.selection);
  }
  if (sanitized.cohort === "exploratory") {
    for (const field of ["protocolMode", "sourceStudyId", "selection"]) {
      if (!Object.hasOwn(sanitized, field)) {
        throw new StatusValidationError(
          `exploratory cohort requires research.${field}`,
        );
      }
    }
    if (sanitized.selection.exploratoryArm !== "scaled") {
      throw new StatusValidationError(
        "research.selection.exploratoryArm must equal scaled",
      );
    }
  }
  if (sanitized.cohort === "mechanism") {
    if (sanitized.protocolMode !== "post_outcome_mechanism_screening") {
      throw new StatusValidationError(
        "mechanism cohort requires post_outcome_mechanism_screening protocol mode",
      );
    }
    if (sanitized.confirmatory !== false) {
      throw new StatusValidationError(
        "mechanism cohort requires research.confirmatory to equal false",
      );
    }
    if (sanitized.studyId !== MECHANISM_STUDY_ID) {
      throw new StatusValidationError(
        `mechanism cohort requires research.studyId to equal ${MECHANISM_STUDY_ID}`,
      );
    }
    if (sanitized.sourceStudyId !== MECHANISM_SOURCE_STUDY_ID) {
      throw new StatusValidationError(
        `mechanism cohort requires research.sourceStudyId to equal ${MECHANISM_SOURCE_STUDY_ID}`,
      );
    }
    if (!MECHANISM_PHASE_LABELS.has(sanitized.phase)) {
      throw new StatusValidationError("mechanism cohort requires research.phase to equal 8 or 9");
    }
    if (sanitized.phaseLabel !== MECHANISM_PHASE_LABELS.get(sanitized.phase)) {
      throw new StatusValidationError(
        "mechanism cohort phaseLabel does not match its frozen phase",
      );
    }
    if (sanitized.totalRuns !== 32) {
      throw new StatusValidationError("mechanism cohort requires research.totalRuns to equal 32");
    }
    const mechanismArmIds = sanitized.arms.map((arm) => arm.id);
    if (
      mechanismArmIds.length !== MECHANISM_ARMS.size ||
      new Set(mechanismArmIds).size !== MECHANISM_ARMS.size ||
      [...MECHANISM_ARMS].some((arm) => !mechanismArmIds.includes(arm)) ||
      sanitized.arms.some((arm) => arm.totalRuns !== 8)
    ) {
      throw new StatusValidationError(
        "mechanism cohort requires four unique frozen arms with eight runs each",
      );
    }
    if (
      sanitized.currentRun !== null &&
      (sanitized.currentRun.cohort !== "mechanism" ||
        sanitized.currentRun.target !== 100_000 ||
        !MECHANISM_ARMS.has(sanitized.currentRun.arm))
    ) {
      throw new StatusValidationError(
        "mechanism currentRun requires a frozen arm, cohort mechanism, and target 100000",
      );
    }
  }
  return sanitized;
...
export function decorateStatus(status, now = new Date()) {
  ...
  const researchComplete =
    status.research?.state === "complete" &&
    (status.research.phase === 7 || status.research.phase === 9);
